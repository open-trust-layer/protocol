...
pub fn process_relationship_operation(input:&Json)->Result<Json,OlpError>{
 ...
 let understood=critical_set(input)?;if critical.iter().any(|x|!understood.contains(x)){return Err(OlpError::unsupported("UNSUPPORTED_CRITICAL_RELATIONSHIP_QUALIFIER","unsupported critical relationship qualifier"));}let mut uninterpreted:Vec<String>=qualifiers.keys().filter(|x|!understood.contains(*x)).cloned().collect();uninterpreted.sort_by(|x,y|x.as_bytes().cmp(y.as_bytes()));
